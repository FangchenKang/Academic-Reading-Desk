...
- 桌面端横屏工作台：首页包含原文输入、学科标签、默认显示偏好和 AI 解析结果区。
- Mock 兜底结果：未配置 AI Key 时也能完整预览双语精读句卡、核心术语、学术表达词汇、句式和自动笔记。
- 双语精读优先：AI 解析结果区按“双语精读 / 核心术语 / 学术表达词汇 / 重点句式 / 自动笔记”的顺序呈现；双语精读支持逐句点击展开中文，不会一次性强制展开所有翻译。
- 模块内精读交互：核心术语和学术表达词汇默认显示简短释义，点击单项后展开解释和例句；左侧“英文优先 / 双语默认 / 中文优先”只作为新结果的默认显示偏好，不会覆盖用户在模块内的展开状态。
...
- GitHub 同步接口：已预留 `/api/github/sync`，未配置时返回友好提示；主界面底部只显示“本地已保存 / 已同步到 GitHub”等简洁状态，不展示冗长文件路径。
